...
public class Main {
    ...
    // save to file
    public static void saveTasks(String filename, TaskList data) {
        Gson gson = new Gson();
        try {
    ...
    // load tasks
    public static TaskList loadTasks(String filename) {
        Gson gson = new Gson();
        try {
            FileReader reader = new FileReader(filename);
            TaskList t = gson.fromJson(reader, TaskList.class);
            reader.close();
            return t;
        }
        catch (IOException e) {
    ...
    public static void main(String[] args)  {
        ...
        userChoice = ui.getUserOption(0,8);
        while (userChoice != 0){
            switch (userChoice) {
                case 1:         // Add a Task
                    toDo.addTask();
                ...
                case 6:         // List Tasks in order
                    ...
                    break;
                case 7:         // Retrieve list from file
                    toDo = loadTasks(FILENAME);
                    break;
                case 8:         // Save list to file
                    saveTasks(FILENAME, toDo);
                    break;
            }
            userChoice = ui.getUserOption(0,8);
        }
